---
title: "Distancesanddirections Interactive Directions v2"
description: ""
sidebarTitle: "Distancesanddirections Interactive Directions v2"
---
...
```
https://api.addressy.com/distancesanddirections/interactive/directions/v2/
```
...
```
https://api.addressy.com/distancesanddirections/interactive/directions/v2/wsdl3.ws
```
...
| DistanceType | String | Fastest<br/>Shortest                                                                             | Specifies how the distances between the stores are calculated.                                                                                                                                                                                                           |
| StartDay     | String | NotSupplied<br/>Monday<br/>Tuesday<br/>Wednesday<br/>Thursday<br/>Friday<br/>Saturday<br/>Sunday | The day of the week the route is to start on. Provide this parameter only if you wish to utilise superior 'Speed Profiles' historic traffic speed data over time for avoiding rush hours and such.                                                                       |
| StartTime    | String |                                                                                                  | The time of day the route is to start on, measured in whole minutes from midnight. (E.g. 8:30am is entered as 510.) Provide this parameter only if you wish to utilise superior 'Speed Profiles' historic traffic speed data over time for avoiding rush hours and such. |
...
| StepNumber    | Integer | 0                                        | A zero based counter indicating the row number.    |
| Action        | String  | D                                        | The type of routing instruction.                   |
| Description   | String  | Depart EAST from start on [Moseley Road] | Textual description of the routing instruction.    |
| Road          | String  | [Moseley Road]                           | The name of the road to turn onto.                 |
| StepTime      | Integer | 14                                       | The time in seconds for this part of the route.    |
| StepDistance  | Integer | 149                                      | The distance in metres for this part of the route. |
| TotalTime     | Integer | 14                                       | The total time in seconds for the route.           |
| TotalDistance | Integer | 149                                      | The total distance in metres for the route.        |
...
| 1008 | StartDay invalid                   | The StartDay parameter was not recognised.                                                                                                                                           | Ensure the StartDay parameter is one of the exact given values.               |
| 1009 | StartTime invalid                  | The StartTime parameter was incorrect.                                                                                                                                               | Ensure the StartTime parameter is an integer between 0 and 1439.              |
| 1010 | StartDay or StartTime not provided | If either StartDay or StartTime is provided, both must be provided.                                                                                                                  | Ensure both or neither of the StartDay and StartTime parameters are provided. |
...
```
https://api.addressy.com/DistancesAndDirections/Interactive/Directions/v2.00/dataset.ws?Key=AA11-AA11-AA11-AA11&Start=381600,259400&Finish=380600,258400&WayPoints=391703,251451&DistanceType=Fastest&StartDay=Monday&StartTime=510
```
...
```asp ASP function
function DistancesAndDirections_Interactive_Directions_v2_00(Key, Start, Finish, WayPoints, DistanceType, StartDay, StartTime)
      ...
      'Build the url
      strUrl = "https://api.addressy.com/DistancesAndDirections/Interactive/Directions/v2.00/recordset.ws?"
      ...
      strUrl = strUrl & "&StartDay=" & Server.UrlEncode(StartDay)
      strUrl = strUrl & "&StartTime=" & Server.UrlEncode(StartTime)
      ...
      set DistancesAndDirections_Interactive_Directions_v2_00 = rst
